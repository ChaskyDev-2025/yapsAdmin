...
const GestionFlotas = () => {
  ...
  // Estado para gestionar documentos de una flota específica
  ...
  const handleAddDocument = async () => {
    // Si estamos en el modal de gestión de flota, usar la función específica
  ...
  // Funciones para modal de gestión de documentos por flota
  ...
  const handleSaveFlotaDocs = async () => {
    ...
    try {
      ...
      showAlert("Documentos actualizados exitosamente", "success");
      await fetchFlotas();
      handleCloseDocsManagerModal();
    } catch (error) {
      console.error("Error al actualizar documentos:", error);
      showAlert("Error al actualizar documentos: " + error.message, "error");
    }
  };
  ...
  return (
    <Box>
      ...
      {/* Modal de gestión de documentos de una flota específica */}
      <Dialog 
      ...
      >
        ...
        >
          📂 Gestión de Documentos - {selectedFlotaForDocs?.nombre}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
          ...
          {selectedFlotaForDocs?.documentosFlota?.otrosDocumentos && 
           selectedFlotaForDocs.documentosFlota.otrosDocumentos.length > 0 ? (
            <TableContainer component={Paper} variant="outlined">
              <Table size="small">
                ...
                <TableBody>
                  {selectedFlotaForDocs.documentosFlota.otrosDocumentos.map((doc, index) => (
                    <TableRow key={index}>
                      <TableCell sx={{ fontFamily: "Mulish, sans-serif" }}>{doc.tipo}</TableCell>
                      <TableCell sx={{ fontFamily: "Mulish, sans-serif" }}>{doc.nombre}</TableCell>
                      <TableCell sx={{ fontFamily: "Mulish, sans-serif", maxWidth: "200px" }}>
                      ...
                      <TableCell align="center">
                        ...
                        <IconButton
                          size="small"
                          color="error"
                          onClick={() => handleDeleteDocFromFlota(index, doc.nombre || doc.tipo)}
                          title="Eliminar"
                        >
                          <DeleteIcon fontSize="small" />
                        </IconButton>
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button 
            onClick={handleCloseDocsManagerModal}
            sx={{ fontFamily: "Mulish, sans-serif", fontWeight: 600, color: "#484848" }}
          >
            Cerrar
          </Button>
          <Button 
            onClick={handleSaveFlotaDocs}
            variant="contained" 
            sx={{ 
              bgcolor: "#d7171a",
              fontFamily: "Mulish, sans-serif",
